    ...
    F: WorkerFnType + 'static,
{
    ...
    // Sometimes users use ianaio-web-starter_worker directly, sometimes it is imported via ianaio-web-starter.
    // We inspect Cargo.toml to find out.
    pub fn worker_crate_name() -> syn::Path {
        if let Ok(m) = crate_name("ianaio-web-starter-worker") {
            return match m {
                FoundCrate::Itself => parse_quote!(ianaio-web-starter_worker),
                FoundCrate::Name(m) => {
        ...
        if let Ok(m) = crate_name("ianaio-web-starter") {
            return match m {
                FoundCrate::Itself => parse_quote!(ianaio-web-starter::worker),
                FoundCrate::Name(m) => {
                    let crate_name = Ident::new(&m, Span::mixed_site());
