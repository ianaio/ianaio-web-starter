...
use ianaio-web-starter_file::{callbacks::read_as_text, Blob, File, ObjectUrl};
...
async fn text_future() {
    ...
    assert_eq!(
        ianaio-web-starter_file::futures::read_as_text(&blob).await.unwrap(),
        "hello"
    );
}
...
async fn data_url_future() {
    ...
    assert_eq!(
        ianaio-web-starter_file::futures::read_as_data_url(&blob).await.unwrap(),
        PNG_FILE_DATA
    );
}
...
async fn bytes_future() {
    let blob = Blob::new_with_options(PNG_FILE, Some("image/png"));
    assert_eq!(
        ianaio-web-starter_file::futures::read_as_bytes(&blob).await.unwrap(),
        PNG_FILE
    );
}
...
async fn blob_to_url() {
    ...
    let body = ianaio-web-starter_file::futures::read_as_text(&body.into())
        .await
        .unwrap();
