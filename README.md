...
<p align="center">
  <img src="https://github.com/ianaio/ianaio-web-starter/blob/master/website/static/img/IanaIO-Web-Starter-Logo.svg" width="400" height="300" />
</p>
...
IanaIO Web Starter is a collection of libraries, and those libraries provide ergonomic Rust 
wrappers for browser APIs. `web-sys`/`js-sys` are very difficult/inconvenient 
to use directly so ianaio-web-starter provides wrappers around the raw bindngs which makes it easier
...
IanaIO Web Starter is this modular toolkit.
...
* **Support both whole Web applications and small, targeted libraries:** IanaIO Web Starter,
...
* **Cultivate the Rust and Wasm library ecosystem:** We want to use IanaIO Web Starter as a
...
* **Modular Toolkit, not Framework:** IanaIO Web Starter should be a loose collection of
  utility crates that can be used individually, or all together. IanaIO Web Starter doesn't
  ...
  can. Wherever possible, IanaIO Web Starter should prefer interfaces over implementations, so
...
* **Small:** Small code size for faster page loads. No accidentally pulling in
  ...
  make a trade off between using IanaIO Web Starter libraries and having small Wasm binaries.
...
This example uses `ianaio-web-starter::events` for adding event listeners and `ianaio-web-starter::timers`
...
```rust
use ianaio-web-starter::{events::EventListener, timers::callback::Timeout};
...
Want to help us build IanaIO Web Starter? Check out [`CONTRIBUTING.md`](./CONTRIBUTING.md)!
